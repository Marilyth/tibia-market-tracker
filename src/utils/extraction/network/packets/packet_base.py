...
class PacketBase:
    ...
    def from_packet(self, packet: bytes) -> 'PacketBase':
        ...
        """
        self.packet = packet
        self.packet_code = self.read_byte()
    ...
    def write_format(self, fmt: str, value: object) -> None:
        ...
        self.write_bytes(packed_value)

    def read_format(self, fmt: str, length: int) -> object:
        ...
        """
        return struct.unpack(fmt, self.get_bytes(length))[0]
    ...
    def read_string(self, length: int) -> str:
        ...
        return self.read_format(f"{length}s", length).decode("utf-8")

    def write_prefixed_string(self, value: str) -> None:
        """Writes a string to the packet, prefixed by its length.
        ...
        """
        encoded_value = value.encode("utf-8")
        self.write_short(len(encoded_value))
        self.write_bytes(encoded_value)
