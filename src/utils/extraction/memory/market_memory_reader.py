...
class MarketMemoryReader:
    ...
    def find_current_memory(self, buy_offer: int, sell_offer: int, max_buy_offer: int, max_sell_offer: int, item_id: int):
        """Filters the readers with the current values. If all readers only have 1 value left, it is done filtering.
    ...
    def get_current_market_values(self, name: str, throw_on_duplicate: bool = False, scan_run: bool = False) -> MarketValues:
        ...
        Args:
            ...
            throw_on_duplicate (bool, optional): If True, throws an exception if the current memory is a duplicate of the previous one. Defaults to False.
            scan_run (bool, optional): If True, the memory reader is filling the properties with the current values without checking duplicates.
        ...
        if not scan_run and (not name.lower() == "golden helmet" and \
            ...
            self.reset()
            raise Exception(f"The memory address might have changed: {item_id=},{self.last_id=},{buy_offer=},{sell_offer=},{amount_bought=},{amount_sold=},{buy_timestamp=},{sell_timestamp=},{item_ids=},{was_duplicate=},{self.last_id}")
